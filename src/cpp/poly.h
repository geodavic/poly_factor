...
template <typename T>
std::string Polynomial<T>::str() const {
    const char* plus = "";
    const char* space = "";
...
template <typename T>
int polydivide(const Polynomial<T> p, const Polynomial<T> dd, Polynomial<T> &q, Polynomial<T> &r){
    int i,j;
    ...
    if constexpr (std::is_same_v<T, mpz_class>){
        if ( !d.monic() || !p.monic() )
            throw std::invalid_argument( "Division of non-monic integer polynomials is not supported." );
    }
    ...
    for (i = 0; i < offset; i++){
        ...
        if (q.coeffs[i] != 0) {
            for (j = offset; j < len; j++){
                q.coeffs[i+j-offset+1] = q.coeffs[i+j-offset+1] - d.coeffs[len-j-1]*q.coeffs[i];
            }
        }
    }
...
Polynomial<T> gcd(const Polynomial<T> p1, const Polynomial<T> p2){
    ...
    int count = 0;
    while( polydivide(a,b,q,r) && count < p_len ) { // While remainder of a/b is nonzero
        count ++;
        a = b;
        b = r;
    }
    ...
    for (i = 0; i < p_len; i++) 
        g.coeffs[i] = b.coeffs[i]/leading_coeff; // Force to be monic, this is okay since a and b are assumed to be monic
    
    // If over Z, clear denominators of gcd and multiply back in gcd of coefficients of p1 and p2
    if constexpr (std::is_same_v<T, mpz_class>) { 
